...
extern "C" {
  ...
  __global__ void oommf_slonc_deltaMKern(float* __restrict__ sttx, float* __restrict__ stty, float* __restrict__ sttz, 
					 ...
					 float* __restrict__ polFreMsk, float* __restrict__ polFixMsk,
					 float* __restrict__ lambdaFreMsk, float* __restrict__ lambdaFixMsk,
					 ...
					 int NPart) 
  {
    ...
    if (I < NPart){ // Thread configurations are usually too large...
      ...
      float lambdaFre = (lambdaFreMsk == NULL) ? lambdaFreMul : lambdaFreMul * lambdaFreMsk[I]; 
      float lambdaFre2 = lambdaFre * lambdaFre;
      float lambdaFix = (lambdaFixMsk == NULL) ? lambdaFixMul : lambdaFixMul * lambdaFixMsk[I]; 
      float lambdaFix2 = lambdaFix * lambdaFix;
      // Need to do calculation of q_plus, q_minus, A_plus and A_minus
      float lamFrePlus = sqrt(lambdaFre2 + 1);
      float lamFreMinus = sqrt(lambdaFre2 - 1);
      float lamFixPlus = sqrt(lambdaFix2 + 1);
      float lamFixMinus = sqrt(lambdaFix2 - 1);
      float plus_ratio = lamFrePlus / lamFixPlus;
      float minus_ratio = 1.0;
      if (lamFreMinus > 0) { minus_ratio = lamFixMinus / lamFreMinus; }

      // Get polarization factors and compute q_plus and q_minus
      float polFre = (polFreMsk == NULL) ? polFreMul : polFreMul * polFreMsk[I]; 
      float polFix = (polFixMsk == NULL) ? polFixMul : polFixMul * polFixMsk[I]; 
      float plus_factor = 0.5 * polFix * lambdaFix2 * plus_ratio;
      float minus_factor = 0.5 * polFre * lambdaFre2 * minus_ratio;
      float q_plus = plus_factor + minus_factor;
      float q_minus = plus_factor - minus_factor;
      float lplus2 = lamFrePlus * lamFixPlus;
      float lminus2 = lamFreMinus * lamFixMinus;
  ...
  void oommf_slonc_async(float** sttx, float** stty, float** sttz, 
			 ...
			 CUstream* stream)
  {
    ...
    float2 pre = make_float2(beta, pre_field);
    ...
    for (int dev = 0; dev < nDev; dev++) {
      gpu_safe(cudaSetDevice(deviceId(dev)));
	    oommf_slonc_deltaMKern<<<gridSize, blockSize, 0, cudaStream_t(stream[dev])>>> (sttx[dev], stty[dev], sttz[dev],  
										       ...
										       alphamsk[dev],
										       t_flmsk[dev],
										       polFreMsk[dev],polFixMsk[dev],
										       lambdaFreMsk[dev],lambdaFixMsk[dev],
