...
///@internal
__global__ void QiDiffKern(float* __restrict__ Qi,
        ...
        if (j < size.y && k < size.z){ // 3D now:)
            ...
            float Q_i = (Q != NULL) ? Q[x0] * QMul : QMul;
...
#define BLOCKSIZE 16
__export__ void Q_async(float** Qi,
	...
	for (int dev = 0; dev < nDev; dev++) {
		...
		for (int i = 0; i < sx; i++) {
		    QiDiffKern <<<gridSize, blockSize, 0, cudaStream_t(stream[dev])>>> (Qi[dev],
