...
func init() {
	RegisterModule("relaxation", "Landau-Lifshitz-Gilbert equation without precession term", LoadRelax)
}

// Purpose of this relaxation module is to have a solver that ignores the precession term in LLG
...
// 
type relaxUpdater struct {
	τ, m, H, α, γ *Quant
}

func (u *relaxUpdater) Update() {
	...
	gpu.Gilbert(u.τ.Array(), u.m.Array(), u.H.Array(), u.α.Array(), float32(u.α.Multiplier()[0]))
}
