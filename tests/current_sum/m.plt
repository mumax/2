...
set title "Single current density/Sum of current densities validation" 
...
GNUTERM = "wxt"
plot 'm-one.txt' u ($1*1e9):2 t "mx/one" w l lw 2, 'm-sum.txt' u ($1*1e9):2 t "mx/sum",'m-one.txt' u ($1*1e9):3 t "my/one" w l lw 2, 'm-sum.txt' u ($1*1e9):3 t "my/sum",'m-one.txt' u ($1*1e9):4 t "mz/one" w l lw 2, 'm-sum.txt' u ($1*1e9):4 t "mz/sum"
#    EOF
